...
class Gift(models.Model):
    """インフルエンサー宛のギフト通知モデル"""
    sender_name     = models.CharField('送信者名', max_length=100)
    to_profile      = models.ForeignKey(Influencer, on_delete=models.CASCADE,
                                        related_name='received_gifts',
                                        verbose_name='受信インフルエンサー')
    wish_item       = models.ForeignKey(WishItem, on_delete=models.CASCADE,
                                        related_name='gifts',
                                        verbose_name='対象WishItem',
                                        null=True, blank=True)
    tracking_number = models.CharField('追跡番号', max_length=100)
    sent_date       = models.DateField('送付日', null=True, blank=True)
    ...
    anonymous_fee    = models.PositiveIntegerField(default=0)
    wrapping_fee     = models.PositiveIntegerField(default=0)
    message_card_fee = models.PositiveIntegerField(default=0)
    payment_fee      = models.PositiveIntegerField(default=0)

    # ↓ ここから追加
    is_paid = models.BooleanField('決済完了', default=False)
    paid_at = models.DateTimeField('決済日時', null=True, blank=True)
    # ↑ ここまで
